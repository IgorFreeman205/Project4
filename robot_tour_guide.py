...
def main():
  ...
  while True:
    ...
    if frame is None:
        ...
        time.sleep(1)
    if y_pred not in labels:
        waypoint_index = labels.index(y_pred)
        next_waypoint = waypoints[waypoint_index]

    if y_pred == 0:
        print("Detected '0'. Returning to origin...")
        plan_to_pose(0, 0, robot)
        turn_to_theta(0, robot)
        print("Execution stopped.")
        break
    plan_to_pose(next_waypoint[0], next_waypoint[1], robot)
    turn_to_theta(next_waypoint[2], robot)
