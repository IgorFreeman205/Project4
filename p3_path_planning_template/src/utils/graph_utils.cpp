...
void initGraph(GridGraph& graph)
{
    ...
    //It should create space to store data for each node in the graph and initialize any data you might need.
    // *** Task: Initialize any variables YOU added to the GridGraph *** //
    int width = graph.width;
    int height = graph.height;

    graph.node.clear();
    graph.node.resize(width * height);
    for (int i =0; i < width; i++)
    {
        for (int j = 0; j < height; j++)
        {
            ...
            current.city = "";
            current.parent = "-1";
            current.distance = 1000000;
            current.visited = false;
            graph.node[idx] = current;
        }
    }
    // *** End student code *** //
}
...
if (idx < 0 || idx >= graph.node.size())
    {
        std::cerr << "ERROR: getParent: Index " << idx << " is out of bounds." << std::endl;
        return -1;
    }
    const std::string& parent_str = graph.node[idx].parent;
    int parent_idx = std::stoi(parent_str);
    return parent_idx;
    // *** End student code *** //
}
